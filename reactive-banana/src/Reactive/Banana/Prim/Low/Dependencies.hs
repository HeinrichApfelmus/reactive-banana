...
changeParent child parent = [ChangeParentTo (P child) (P parent)]
...
applyChange (InsertEdge parent child) _ =
    parent `doAddChild` child
applyChange (ChangeParentTo child parent) _ = do
    child `doChangeParent` parent
...
doChangeParent :: SomeNode -> SomeNode -> IO ()
doChangeParent (P child) (P parent) = do
