...
import OpenTelemetry.Trace (makeTracer, getGlobalTracerProvider, InstrumentationLibrary (InstrumentationLibrary), inSpan, defaultSpanArguments, tracerOptions)
...
runStep EventNetwork{actuated, s} f = whenFlag actuated $ instrument "runStep" $ do
  output <-
    mask $ \restore -> do
      s1 <- takeMVar s -- read and take lock
      -- pollValues <- sequence polls    -- poll mutable data
      (output, s2) <-
        restore (instrument "f" (f s1)) -- calculate new state
          `onException` putMVar s s1 -- on error, restore the original state
      putMVar s s2 -- write state
      return output
  instrument "output" output -- run IO actions afterwards
  where
